...
///
/// `cache` holds `1 << (H_PRIME - XMSS_CHECKPOINT)` subtree roots (each `N`
/// bytes), indexed by subtree index `start_idx >> XMSS_CHECKPOINT`. Subtrees at
/// height `XMSS_CHECKPOINT` and above are rebuilt from the cache; shallower
/// subtrees fall back to the (uncached) [`xmss_treehash`]. Produces
/// byte-identical output to [`xmss_treehash`] for the same inputs.
#[cfg(feature = "std")]
pub fn xmss_treehash_cached<P: Params>(
    sk_seed: &[u8],
    hash_ctx: &Sha256Ctx,
    adrs: &mut Adrs,
    target_height: u32,
    ...
    let ck = P::XMSS_CHECKPOINT as u32;

    if target_height < ck {
        return xmss_treehash::<P>(sk_seed, hash_ctx, adrs, target_height, start_idx);
    }

    if target_height == ck {
        let sub = (start_idx >> ck) as usize;
        let offset = sub * P::N;
    ...
    let left =
        xmss_treehash_cached::<P>(sk_seed, hash_ctx, adrs, target_height - 1, start_idx, cache);
    let right = xmss_treehash_cached::<P>(
        sk_seed,
        hash_ctx,
        adrs,
        target_height - 1,
...
/// Authentication path for leaf `idx` using a checkpoint cache.
/// Signing-only (`std`). Returns `h_prime * N` bytes.
#[cfg(feature = "std")]
pub fn xmss_auth_path_cached<P: Params>(
    sk_seed: &[u8],
    hash_ctx: &Sha256Ctx,
    adrs: &mut Adrs,
    h_prime: u32,
    ...
    for i in 0..h_prime {
        let sibling_start = ((idx ^ (1 << i)) >> i) << i;
        let tmp = xmss_treehash_cached::<P>(sk_seed, hash_ctx, adrs, i, sibling_start, cache);
        auth.extend_from_slice(&tmp);
    }
    auth
}

/// [`xmss_sign`] using a checkpoint cache for the authentication path.
/// Signing-only (`std`). Returns `XMSS_SIGN_LEN` bytes.
#[cfg(feature = "std")]
pub fn xmss_sign_cached<P: Params>(
    message: &[u8],
    sk_seed: &[u8],
    sk_prf: &[u8],
    pk_seed: &[u8],
    ...
    let auth = xmss_auth_path_cached::<P>(sk_seed, hash_ctx, adrs, h_prime, idx, cache);
...
/// Precompute the XMSS subtree roots at [`Params::XMSS_CHECKPOINT`] for a fixed
/// tree addressed by the current `adrs` layer/tree fields.
///
/// Returns `1 << (H_PRIME - XMSS_CHECKPOINT) * N` bytes of checkpoint subtree
/// roots, in index order. Signing-only (`std`).
#[cfg(feature = "std")]
pub fn xmss_checkpoint_roots<P: Params>(
    sk_seed: &[u8],
    hash_ctx: &Sha256Ctx,
    adrs: &mut Adrs,
) -> Vec<u8> {
    let ck = P::XMSS_CHECKPOINT as u32;
    let count = 1usize << (P::H_PRIME - P::XMSS_CHECKPOINT);
    let mut roots = Vec::with_capacity(count * P::N);
    for sub in 0..count as u32 {
        // Bottom leaf index of the height-ck subtree `sub`.
        let start_idx = sub << ck;
        let node = xmss_treehash::<P>(sk_seed, hash_ctx, adrs, ck, start_idx);
